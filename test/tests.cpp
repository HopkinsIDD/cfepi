...
struct sir_infection_event : public cfepi::interaction_event<sir_epidemic_states> {
  constexpr sir_infection_event(cfepi::person_t p1,
    cfepi::person_t p2,
...
TEST_CASE("SIR model works modularly", "[sir_generator]") {
  ...
  cfepi::filtration_setup<sir_epidemic_states, any_sir_event>{
...
TEST_CASE("Full stack test works", "[sir_generator]") {
  ...
  for (cfepi::epidemic_time_t t = 0UL; t < 365; ++t) {
    ...
    const auto single_event_type_run = [&t,
      ...
      auto event_range_generator =
        cfepi::single_type_event_generator<sir_epidemic_states, any_sir_event, event_index>(
          current_state);
      ...
      ranges::for_each(filtered_view, [&setups_by_filter, &counter](const auto &x) {
        if (cfepi::any_state_check_preconditions<any_sir_event, sir_epidemic_states>{
...
struct seir_exposure_event : public cfepi::interaction_event<seir_epidemic_states> {
  constexpr seir_exposure_event(cfepi::person_t p1,
    cfepi::person_t p2,
    cfepi::epidemic_time_t _time) noexcept
...
TEST_CASE("SEIR model works modularly", "[sir_generator]") {
  ...
  auto do_nothing = [](auto &param __attribute__((unused)),
                      std::default_random_engine &rng __attribute__((unused))) { return; };
  cfepi::filtration_setup<seir_epidemic_states, any_seir_event>{
    initial_conditions, always_true_event, always_true_state, do_nothing
  };
