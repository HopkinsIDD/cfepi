...
\node[draw, fill = green!30, circle] (R) at (2,0) {R};
...
\node[draw, fill = red!30, circle,radius=.1] (I1) at (0,2) {I1};
\node[draw, fill = green!30, circle] (I2) at (2,0) {I2};
\draw[->] (S) -> node[above] {$\gamma$} (I2);
\draw[dashed] (S) -- (I1) ; 
\end{tikzpicture}
```