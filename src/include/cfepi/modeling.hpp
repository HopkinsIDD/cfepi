...
auto single_timestep_run(auto &results,
  ...
  auto &resets)
{
  ...
  const auto single_event_type_run =
    [&all_event_types, &t, &setups_by_filter, &random_source_1, &current_state, &event_probabilities](
  ...
  const auto seeded_single_event_type_run = [&single_event_type_run, &simulation_seed](const auto event_index) {
    ++simulation_seed;
  ...
  auto states_next = ranges::to<std::vector>(ranges::views::transform(setups_by_filter, [t, &random_source_1](auto &x) {
  ...
  size_t simulation_seed = 2)
{
  ...
  auto setups_by_filter = ranges::to<std::vector>(ranges::views::transform(filters, [&current_state](auto &x) {
    return (filtration_setup<states_t, any_event>{ current_state, std::get<0>(x), std::get<1>(x), std::get<2>(x) });
  }));

  ;

  std::vector results{ { ranges::to<std::vector>(
    ranges::views::transform(setups_by_filter, [](const auto &x) { return (aggregate_state(x.current_state)); })) } };
  // results.clear();
  results.reserve(static_cast<size_t>(epidemic_duration + 1));

  for (epidemic_time_t t = 0UL; t < epidemic_duration; ++t) {
    single_timestep_run<states_t, any_event_type, any_event>(results,
      setups_by_filter,
      current_state,
      all_event_types,
      t,
