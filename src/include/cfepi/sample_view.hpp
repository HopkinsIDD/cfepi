...
namespace probability {
  ...
  concept uniform_random_number_engine = \
    ...
    requires(E e, E::result_type s, E&& v, const E x, const E y){
					     {E()};
					     {E(s)};
					     {E(x)};
					     {E(e)};
  ...
  requires
  uniform_random_number_engine<std::remove_reference_t<Gen> > &&
  std::ranges::view<R>
  ...
  private:
    ...
    struct iterator {
      ...
      iterator() = default;
      constexpr iterator(
			 std::ranges::iterator_t<Base> begin,
			 Base* base,
			 Gen gen,
    ...
    struct sentinel {
      ...
      sentinel() = default;
      constexpr sentinel(const Base* base) :
	end_(std::ranges::end(*base)) {}
  ...
  public:
    ...
    constexpr auto end() const {
      return(sentinel<true>{&base_});
    }
    /*
    constexpr auto begin() const requires cor3ntin::rangesnext::simple_view<R> {
      cursor<true> rc(std::begin(base_), &base_, gen_, dist_);
      return(rc);
    }
    constexpr auto end() const  requires cor3ntin::rangesnext::simple_view<R> {
      cursor<true> rc(std::end(base_), &base_, gen_, dist_);
      return(rc);
    }
    constexpr auto begin() requires (!cor3ntin::rangesnext::simple_view<R>) {
      cursor<false> rc(std::begin(base_), &base_, gen_, dist_);
      return(rc);
    }
    constexpr auto end() requires (!cor3ntin::rangesnext::simple_view<R>) {
      cursor<false> rc(std::end(base_), &base_, gen_, dist_);
      return(rc);
    }
    */

    sample_view() = default;
    sample_view(const R& base, double probability): base_(std::move(base)), gen_(), dist_(probability){}
    sample_view(const R& base, double probability, Gen gen): base_(std::move(base)), gen_(gen), dist_(probability) {}
  };
  template<class V, class Gen> sample_view(V&&, double, Gen) -> sample_view<std::ranges::views::all_t<V> , Gen>;
  ...
  namespace views {
	...
	constexpr auto operator()(double probability, Gen gen) const {
	  return([this, probability, gen](auto&& r){return(this->operator()(r, probability, gen));});
	}

      };

      template<std::ranges::viewable_range T>
      constexpr auto operator|(T&& t, auto s){
