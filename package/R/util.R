...
calculate_residual = function(counterfactual_frame,base_name='None_None'){
  #' @importFrom tidyr spread
  all_scenarios = unique(counterfactual_frame$scenario)
  counterfactual_frame = spread(counterfactual_frame,scenario,value)
  ...
  if('Uncontrolled' %in% names(counterfactual_frame)){
    warning("Uncontrolled should not be an epidemic scenario.")
  }
  counterfactual_frame$Uncontrolled = counterfactual_frame[[base_name]]
  for(scenario in all_scenarios){
    counterfactual_frame[[scenario]] = counterfactual_frame[[scenario]] - counterfactual_frame$Uncontrolled
  }
  #' @importFrom tidyr gather_
  counterfactual_frame = gather_(counterfactual_frame,key_col='scenario',value_col='value',gather_cols=all_scenarios)
  counterfactual_frame = select(counterfactual_frame,-Uncontrolled)
